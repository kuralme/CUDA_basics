...
int main() {
  ...
  int *hostMatrix = new int[N * N];
  int *hostResult = new int[N * N];
  init_matrix(hostMatrix, N);

  // Size of the mask in bytes
  size_t bytes_mask = MASK_DIM * MASK_DIM * sizeof(int);

  // Allocate the mask and initialize it
  int *hostMask = new int[MASK_DIM * MASK_DIM];
  init_matrix(hostMask, MASK_DIM);

  // Allocate device memory
  int *deviceMatrix;
  int *deviceResult;
  cudaMalloc(&deviceMatrix, bytes_in);
  cudaMalloc(&deviceResult, bytes_in);

  // Copy data to the device
  cudaMemcpy(deviceMatrix, hostMatrix, bytes_in, cudaMemcpyHostToDevice);
  cudaMemcpyToSymbol(mask, hostMask, bytes_mask);
  ...
  convolution_2d<<<grid_dim, block_dim>>>(deviceMatrix, deviceResult, N);
  cudaMemcpy(hostResult, deviceResult, bytes_in, cudaMemcpyDeviceToHost);

  auto duration = std::chrono::duration_cast<std::chrono::microseconds>
    (std::chrono::high_resolution_clock::now() - begin);
  verify_result(hostMatrix, hostMask, hostResult, N);
  printf("Convolution2D took\t%ld[us]\n", duration.count());
